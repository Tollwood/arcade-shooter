...
public class DualJoystickPlayerController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            handleMovementOnly(xMovementLeftJoystick,zMovementLeftJoystick);
        }

        // if there is only input from the right joystick
        if (leftJoystickInput == Vector3.zero && rightJoystickInput != Vector3.zero)
        {
            handleRotationOnly(xMovementRightJoystick, zMovementRightJoystick);
        }

        // if there is input from both joysticks (Left And Right)
        if (leftJoystickInput != Vector3.zero && rightJoystickInput != Vector3.zero)
        {
            handleMovementAndRotation(xMovementLeftJoystick, zMovementLeftJoystick, xMovementRightJoystick, zMovementRightJoystick);
        }
    }

    private void handleMovementAndRotation(float xMovementLeftJoystick, float zMovementLeftJoystick, float xMovementRightJoystick,float zMovementRightJoystick)
    {
...
*/
    ...
    private void handleRotationOnly(float xMovementRightJoystick, float zMovementRightJoystick)
    {
        // calculate the player's direction based on angle
    ...
    private void handleMovementOnly(float xMovementLeftJoystick, float zMovementLeftJoystick)
    {
        // calculate the player's direction based on angle
